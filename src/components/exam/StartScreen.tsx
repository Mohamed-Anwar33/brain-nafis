            ...
            <div className="relative z-10">
              ...
              <div className="text-center mb-8 sm:mb-16">
                ...
                <div className="space-y-4 sm:space-y-6 mb-8 sm:mb-12">
                  <h2 className="text-xl sm:text-3xl md:text-5xl font-black text-slate-800">المتوسطة <span className="text-indigo-600">الرابعة والعشرون</span> بجدة</h2>
              ...
              {existingName ? (
                <div className="space-y-6 sm:space-y-10 text-center bg-white/60 p-8 sm:p-12 rounded-[2.5rem] sm:rounded-[3.5rem] border border-slate-100 backdrop-blur-xl shadow-lg">
                  <div className="space-y-3 sm:space-y-4">
                    <h3 className="text-2xl sm:text-4xl md:text-5xl font-black text-slate-900">
                    ...
                    <p className="text-lg sm:text-2xl text-slate-500 font-bold tracking-wide">هل أنت مستعد للانطلاق في رحلتك المبهجة اليوم؟</p>
                  </div>
                  ...
                  <div className="grid gap-4 sm:gap-6">
                    <Button
                      ...
                      disabled={isLoading}
                      className="w-full h-16 sm:h-24 text-xl sm:text-3xl font-black rounded-2xl sm:rounded-3xl bg-gradient-to-r from-emerald-600 to-teal-500 hover:scale-[1.02] active:scale-95 transition-all shadow-2xl shadow-emerald-600/20"
                    >
                      {isLoading ? (
                        <span className="flex items-center gap-3 text-white">
                          <Brain className="animate-spin h-7 w-7 sm:h-10 sm:w-10" />
                          جاري التحميل...
                        </span>
                      ) : (
                    ...
                    {onLogout && (
                      <Button
                        ...
                        onClick={onLogout}
                        disabled={isLoading}
                        className="w-full h-14 sm:h-20 text-lg sm:text-2xl font-bold rounded-2xl sm:rounded-3xl text-slate-400 hover:text-slate-900 hover:bg-slate-100"
                      >
                ...
                <form onSubmit={handleSubmit} className="space-y-8 sm:space-y-10">
                  <div className="space-y-3 sm:space-y-4">
                    ...
                    {error && (
                      <p className="text-rose-500 text-sm sm:text-xl font-bold animate-pulse mr-4">{error}</p>
                    )}
                  </div>
