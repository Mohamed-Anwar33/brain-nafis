...
import { Trash2, Plus, Loader2, ArrowUpDown, X, ArrowUp, ArrowDown, Edit, RotateCcw, Save } from "lucide-react";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export default function CentralExamOrdering() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (questionForm.id) {
        const { error } = await supabase.from("ordering_game_questions").update(payload).eq("id", questionForm.id);
        if (error) throw error;
        toast.success("تم التحديث بنجاح");
      } else {
        const { error } = await supabase.from("ordering_game_questions").insert(payload);
        if (error) throw error;
  ...
  const handleEdit = (q: OrderingQuestion) => {
    const paddedItems = [...q.items];
    while (paddedItems.length < 3) {
      paddedItems.push({ text: "", order: paddedItems.length + 1 });
    }
    setQuestionForm({
      id: q.id,
      items: paddedItems
  ...
  const addItem = () => {
    setQuestionForm({
      ...questionForm,
      items: [...questionForm.items, { text: "", order: questionForm.items.length + 1 }],
    });
  };

  const removeItem = (index: number) => {
    if (questionForm.items.length <= 2) {
      toast.error("يجب الاحتفاظ بعنصرين على الأقل");
      return;
    }
    ...
    <div className="space-y-8 pb-10" dir="rtl">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black text-slate-800 tracking-tight">لغز الترتيب</h1>
            <p className="text-slate-500 font-medium mt-1">إعداد أسئلة ترتيب العناصر تصاعدياً أو تنازلياً أو حسب التسلسل الزمني</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card className="bg-gradient-to-br from-emerald-50 to-emerald-100/50 border-emerald-200 shadow-sm overflow-hidden relative">
           <div className="absolute left-0 top-0 w-32 h-32 bg-emerald-500/10 rounded-full blur-3xl -translate-x-1/2 -translate-y-1/2"></div>
          <CardContent className="p-6 relative z-10 flex flex-col items-center">
            <div className="text-4xl font-black text-emerald-600 mb-1">{questions.length}</div>
            <div className="text-sm font-bold text-slate-600">إجمالي الأسئلة</div>
          </CardContent>
        </Card>
        <Card className="bg-gradient-to-br from-teal-50 to-teal-100/50 border-teal-200 shadow-sm overflow-hidden relative">
          <CardContent className="p-6 flex flex-col items-center">
            <div className="text-4xl font-black text-teal-600 mb-1">{questions.filter((q) => q.is_active).length}</div>
      ...
      <Card className="shadow-md border-slate-200 bg-white">
        <CardHeader className="bg-slate-50/50 border-b pb-4">
          <div className="flex items-center justify-between">
            <CardTitle className="flex items-center gap-2 text-xl text-slate-800">
              {questionForm.id ? <Edit className="w-5 h-5 text-amber-500" /> : <Plus className="w-5 h-5 text-emerald-500" />} 
              {questionForm.id ? "تعديل السؤال" : "إضافة سؤال ترتيب جديد"}
            </CardTitle>
            {questionForm.id && (
        ...
        <CardContent className="p-6">
          <form onSubmit={handleSave} className="space-y-6">
             <div className="bg-emerald-50 text-emerald-800 p-4 rounded-xl text-sm mb-4 border border-emerald-100 flex gap-3">
               <ArrowUpDown className="w-5 h-5 shrink-0 mt-0.5" />
               <p>أدخل العناصر <b>بالترتيب الصحيح 100%</b>، سيقوم النظام تلقائياً ببعثرة هذا الترتيب عندما يظهر للطالب اثناء الاختبار، ويطالبه بإعادة ترتيبها كما أدخلتها أنت هنا.</p>
             </div>
            ...
            <div className="space-y-3">
              <Label className="text-sm font-bold text-slate-700">قائمة العناصر (بالترتيب الصحيح):</Label>
              <div className="flex flex-col gap-3">
                {questionForm.items.map((item, index) => (
                  <div key={index} className="flex items-center gap-3 bg-slate-50 p-3 rounded-2xl border border-slate-200 relative group transition-colors hover:border-emerald-300 hover:bg-emerald-50/30">
                    <div className="w-10 h-10 bg-emerald-100 text-emerald-700 rounded-xl flex items-center justify-center font-black shrink-0 shadow-inner">
                      {index + 1}
                    </div>
                    <Input
                      placeholder={`أدخل العنصر رقم ${index + 1}`}
                      value={item.text}
                      onChange={(e) => updateItem(index, e.target.value)}
                      className="h-12 rounded-xl bg-white border-slate-300 focus-visible:ring-emerald-500"
                    />
                    <div className="flex gap-1 shrink-0">
                      <Button type="button" variant="outline" className="h-12 w-10 p-0 rounded-xl bg-white border-slate-200 text-slate-500 hover:text-emerald-700 hover:bg-emerald-50" onClick={() => moveItem(index, -1)} disabled={index === 0}>
                        <ArrowUp className="w-4 h-4" />
                      </Button>
                      <Button type="button" variant="outline" className="h-12 w-10 p-0 rounded-xl bg-white border-slate-200 text-slate-500 hover:text-emerald-700 hover:bg-emerald-50" onClick={() => moveItem(index, 1)} disabled={index === questionForm.items.length - 1}>
              ...
              <Button type="button" variant="outline" onClick={addItem} className="w-full h-12 border-dashed border-2 rounded-2xl gap-2 font-bold text-slate-500 hover:text-emerald-600 hover:bg-emerald-50 hover:border-emerald-300 transition-colors mt-2">
                <Plus className="w-5 h-5" /> إضافة عنصر جديد للترتيب
              </Button>
            </div>
            ...
            <div className="flex justify-end pt-4 border-t">
              <Button 
                ...
                className={`h-12 px-10 text-base font-bold rounded-xl shadow-md ${questionForm.id ? "bg-amber-500 hover:bg-amber-600 shadow-amber-500/20" : "bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 shadow-emerald-500/20"} gap-2`}
              >
      ...
      <div className="space-y-4">
        <h2 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
           قائمة أسئلة الترتيب المضافة
        </h2>
        
        {isLoading ? (
          <div className="flex items-center justify-center py-20 bg-white rounded-2xl border"><Loader2 className="w-10 h-10 animate-spin text-emerald-500" /></div>
        ) : questions.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-slate-300">
            <ArrowUpDown className="w-16 h-16 text-slate-300 mx-auto mb-4" />
                ...
                <Card key={q.id} className="overflow-hidden shadow-sm hover:shadow-md transition-shadow border-slate-200">
                   ...
                   <div className="p-4 space-y-2">
                      {q.items.slice(0, 5).map((item, i) => (
                         <div key={i} className="flex items-center gap-3 text-sm bg-slate-50 rounded-lg p-2 px-3 border border-slate-100">
                            <span className="w-6 h-6 rounded-md bg-emerald-100 text-emerald-700 flex items-center justify-center font-bold text-xs shrink-0">{i + 1}</span>
                            <span className="truncate font-medium flex-1 text-slate-700" title={item.text}>{item.text}</span>
                         </div>
                      ))}
                      {q.items.length > 5 && (
